...
© [Anywhere.tours](https://anywhere.tours)

This privacy policy describes how the Viator "Reply by AI" Chrome Extension ("the Extension") handles information when you use it.
...
## 1. Information the Extension Uses

### 1.1 Stored on your device (Chrome storage)
...
|------|--------|--------|
| Gemini API key | Google (Gemini API) | Authenticate your request to generate reply text |
| Review title, body, reviewer/supplier names | Google (Gemini API) | Generate a suggested reply |

**We (Anywhere.tours) do not operate servers that receive or store this data.** The only external service that receives data when you use "Reply by AI" is **Google’s Gemini API**, subject to [Google’s privacy policy](https://policies.google.com/privacy).
...
- **On your device:** Your API key remains in Chrome’s extension storage until you remove it (e.g. via the "Clear" button in the Extension options) or uninstall the Extension.
- **Google:** Any data sent to the Gemini API is subject to Google’s data practices; we do not control Google’s retention or processing.

---

## 5. Your choices

- You can **clear** your stored API key at any time in the Extension options.
- You can **uninstall** the Extension at any time; this removes the Extension and any data it stored locally.
- You choose **when** to use "Reply by AI"; no data is sent to Gemini until you click that button.
...
- **Anywhere.tours** — [https://anywhere.tours](https://anywhere.tours)
